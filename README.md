...
将「视频流 → 障碍检测 → 细粒度分类 → 结构化场景建模 → 规避决策」整条链路下沉到端侧芯片，不依赖云端推理。针对通用 LLM 直接进入实时闭环导致的时延抖动与输出不可控问题，采用 **双层决策架构**：确定性规则引擎承担实时决策，小参数 LLM 作为非实时策略顾问。
...
| 模型 | 后端 | 延迟 | 说明 |
|------|------|------|------|
| YOLOv8n FP32 | SNPE **GPU** | **715ms** | 持续流下 YOLOv5s 版本为 39ms/帧 |
| YOLOv5s FP32 | SNPE GPU | 1653ms | GPU 持续流 39ms / CPU 256ms |
| InceptionV3 FP32 | SNPE GPU | 31ms | CPU 后端 95ms |
| TinyLlama-1.1B | Mac MPS | 1.4–2.1s/帧 | 板端 CPU >10s/帧，HTP 不可用 |

量化压缩：InceptionV3 92MB → 24MB（**-74%**）；YOLOv5s 29MB → 7.2MB（**-75%**）。

> 注：x86 CPU 上 INT8 并不加速（反而慢 2.7%），因为缺少 INT8 向量指令、需运行时反量化。INT8 的收益必须依赖 DSP/HTP 硬件。
...
**结论**：TinyLlama-1.1B 不具备 if-then 数值比较能力 —— 将 `free_space=0.99` 误解为「0.99 米距离」，且 Chat 微调带来的谨慎偏置压倒 prompt 指令。这是模型能力天花板，而非 prompt 工程问题。据此提出 **约束解码** 改进方向：规则引擎不直接输出动作，而是产出安全候选集，LLM 仅在候选集内选优并生成理由。
...
- 板端 HTP 路径不可用：vendor runtime `v2.0.1.220706112757` 与 SDK `v2.22.6` 版本不匹配，LLM 无法在板端运行（视觉模型已通过 SNPE GPU 后端绕开）。
- 1.1B 级模型不具备结构化数值决策能力，四轮实验已证伪。
